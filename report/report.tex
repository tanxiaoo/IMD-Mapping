% Impervious surface density mapping -- Milan, Hanoi, Ho Chi Minh City, 2018.
% Compile with XeLaTeX (the text carries U+00B7 mid dots and Zgela's name).
% The bibliography needs a BibTeX pass between the LaTeX runs:
%   xelatex report / bibtex report / xelatex report / xelatex report
% `latexmk` does all four in the right order; on Overleaf, set the compiler to
% XeLaTeX and it handles the sequence itself. This directory is self-contained:
% report.tex, refs.bib and figs/ are everything the build needs.
...
\usepackage{float}
\usepackage{url}
% Sober link colours: dark enough to read as black in a greyscale print, and
% distinct from the bright default red/green/cyan hyperref would otherwise use.
\usepackage{xcolor}
\definecolor{darkblue}{RGB}{0,51,120}
\definecolor{darkred}{RGB}{128,0,32}
% Numeric citations in order of first appearance (unsrt), matching the
% bracketed [1]..[15] style the reference report uses. natbib + BibTeX rather
% than biblatex + biber: BibTeX is the default engine on Overleaf for every
% compiler, so the project builds there with no latexmkrc and no per-project
% backend setting, whereas biblatex additionally requires the biber backend to
% be selected. natbib's numbers also need no \citep/\citet distinction here.
\usepackage[numbers,square,sort&compress]{natbib}
\bibliographystyle{unsrtnat}

% For print, swap the two lines below: hidelinks removes the colour boxes.
% \usepackage[hidelinks,breaklinks]{hyperref}
\usepackage[colorlinks,breaklinks,
            linkcolor=darkblue,citecolor=darkblue,urlcolor=darkred]{hyperref}
% Long DOIs and the ArcGIS documentation URL overrun the text block otherwise.
\usepackage{xurl}
% Slashes and underscores in GEE asset ids are legal break points; without
% this a single \texttt{} identifier sets the whole line overfull.
\renewcommand{\UrlBreaks}{\do\/\do-\do_\do.}
\usepackage[capitalise,nameinlink]{cleveref}

% Every figure resolves inside report/. figs/ mirrors the run directories
% (figs/outputs_v2/, figs/outputs_validation/, ...) so the paths below are
% unchanged from the pipeline's own; `python code/sync_figs.py` refreshes the
% copies. No ../ entry: report/ must compile with no file outside it.
\graphicspath{{figs/}}

% Numeric policy: full maths mode. Every signed value is set with a real
% minus, so a negative bias never renders as a hyphen.
\sisetup{
  detect-all,
...
\title{Impervious surface density from Sentinel-2 composites and AlphaEarth
embeddings: Milan, Hanoi and Ho Chi Minh City, 2018}
\author{}
\date{}
...
heat island analysis in the Vietnamese cities \cite[p.~11]{zgela2026}.
...
gives RMSE 14.123, MAE 10.675, \Rsq{} 0.837 and bias $+0.624$ against his reported
14.12, 10.68, 0.837 and $+0.62$; the support vector regressor gives 14.756,
11.400, 0.822 and $+0.274$ against his 14.76, 11.40, 0.822 and $+0.27$ \cite[p.~5]{zgela2026}.
...
\subsection*{What this report finds}
...
on RMSE, MAE and \Rsq{}. The advantage is also mechanistically interpretable: it
...
transferred models score \Rsq{} at or below zero in three of the four city and
predictor combinations, which is no more informative than predicting the mean
everywhere, and local retraining recovers \Rsq{} to between 0.522 and 0.649 in every
...
\subsection{Targets: CLMS and GHS-BUILT-S}

The models are fitted to two different products. Milan uses the Copernicus Land
Monitoring Service imperviousness density layer for 2018 \cite{clms2018imd}, and Hanoi and Ho
Chi Minh City use GHS-BUILT-S for the same year \cite{pesaresi2023ghsl}. Both are supplied at 10\,m and both
...
inherent limitation of the chosen reference dataset \cite[pp.~8, 10]{zgela2026}.
...
\subsection{AlphaEarth embeddings}

The first predictor set is the AlphaEarth satellite embedding \cite{brown2025alphaearth},
\path{GOOGLE/SATELLITE_EMBEDDING/V1/ANNUAL}, a 64-dimensional learned representation
supplied at 10\,m. The collection is filtered to a single year and all 64 bands,
\texttt{A00} to \texttt{A63}, are taken as predictors. The method and its rationale are
Žgela's \cite[pp.~3--5]{zgela2026} and are not re-derived here.
...
every band against the IMD groups by Kruskal-Wallis \cite{kruskal1952} and found 60 of the 64 to
differ significantly across classes, and a separate feature-selection experiment
found the full set to outperform any reduced subset \cite[pp.~3--5, Fig.~3]{zgela2026}. The
...
\subsection{Sentinel-2: search, screening and composites}

The second predictor set is built from Sentinel-2 surface reflectance \cite{esa2018s2},
\path{COPERNICUS/S2_SR_HARMONIZED}, over each city's area of interest for 2018. Ten
...
probability gate at \texttt{MSK\_CLDPRB < 40} on top. A date is kept when cloud cover
...
and Ho Chi Minh City 3. \Cref{fig:composite-depth} shows them on the 2018 calendar.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/fig_composite_depth.png}
  \caption{Usable Sentinel-2 acquisitions per city after cloud screening, 2018.
...
\textbf{Even then Ho Chi Minh City reaches 16 dates against the 17 required. It fails
...
\subsection{Photo-interpreted plots}
...
in Section 6 runs on 450 observations per city. One code, \texttt{Rock/material}, is
...
\begin{itemize}
  \item \textbf{strict}, the primary rule, counts parking, pavement, road, roof, railroad,
        airport and pool as impervious;
  \item \textbf{B} adds permeable pavement, which matches the sealed-surface definition
        CLMS works to;
  \item \textbf{C} adds unpaved dirt road and heavily compacted bare soil, on the ground
...
\subsection{Sampling, spatial blocking and buffering}
...
\cite[pp.~2, 4, Figs.~1--2]{zgela2026}.
...
neighbour index \cite{esri2024ann}, which runs from 1.02 for the 0\,\% class down to 0.75 for the
...
baseline was correctly reconstructed. \Cref{fig:spatial-split} shows the resulting split.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{outputs_v2/fig01_spatial_split.png}
  \caption{Training points, test points and buffer-removed points over the 1\,km
...
\subsection{Estimators, tuning and spatial cross-validation}
...
\cite{breiman2001rf} and a support vector regressor \cite{drucker1996svr}.
Tuning is performed with scikit-learn \cite{pedregosa2011sklearn}. Only estimators whose trained form can be
applied to a raster through the Google Earth Engine Python API \cite{gorelick2017gee} were
...
autocorrelation, and whether it worked is measurable: \Cref{fig:cv-inflation} compares
cross-validation RMSE under spatial folds against random folds, per estimator
and per block size.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/fig_cv_inflation.png}
  \caption{Cross-validation RMSE under spatial blocking against random folds,
...
predicted values $\hat{y}_i$, and with $\bar{y}$ the mean of the reference values:

\begin{equation}
  \mathrm{RMSE} = \sqrt{\frac{1}{n}\sum_{i=1}^{n}\left(y_i - \hat{y}_i\right)^2}
\end{equation}

the root mean squared error, in IMD percentage points;

\begin{equation}
  \mathrm{MAE} = \frac{1}{n}\sum_{i=1}^{n}\left|y_i - \hat{y}_i\right|
\end{equation}

the mean absolute error, in the same units;
...
the proportion of reference variance the map accounts for;

\begin{equation}
  \mathrm{Bias} = \frac{1}{n}\sum_{i=1}^{n}\left(y_i - \hat{y}_i\right)
\end{equation}

the mean signed error, in the same units.
...
\textbf{Bias is observed minus predicted}, reference minus map. The sign convention
is stated here because it is the one thing in this section that a reader can
invert without noticing:

\begin{itemize}
  \item a \textbf{positive} bias means the reference is higher than the map, so the map
        \textbf{under-predicts};
  \item a \textbf{negative} bias means the map reads higher than the reference, so the map
        \textbf{over-predicts}.
\end{itemize}
...
taking the predicted value $\hat{y}_i$ as a proportion for the plug-in estimate of the
plot's true impervious fraction, the binomial variance of the reference estimate
is $\hat{y}_i(1 - \hat{y}_i)/n_i$. Averaging it across plots and removing it from the mean
squared error gives:

\begin{equation}
  \mathrm{RMSE}_{\mathrm{corr}} = \sqrt{\max\left(0,\;
    \mathrm{RMSE}^2 - \frac{1}{n}\sum_{i=1}^{n}
    \frac{\hat{y}_i\left(1 - \hat{y}_i\right)}{n_i}\right)}
\end{equation}

the noise-corrected RMSE, in the same units: an estimate of map error with the
...
$\mathrm{RMSE}_{\mathrm{corr}}$ is an \textbf{upper bound} on map error rather than a point estimate, and
...
should. The true map error is therefore at or below $\mathrm{RMSE}_{\mathrm{corr}}$, and the correct
use of the figure is to say that a map's error is at most some value, never that
it equals it.

\subsection{The two-validation design}
...
\textbf{Same-source validation} scores each map against the product it was trained
...
\textbf{Independent validation} scores every map against the photo-interpreted plots
...
\section{Milan results --- same-source validation}
...
\subsection{Four predictor sets ranked}

\Cref{tab:milan-holdout} gives the holdout metrics for the four Milan predictor
sets under the GEE random forest.

\begin{table}[htbp]
  \centering
  \caption{Milan holdout metrics against CLMS, GEE random forest, $n = 1\,014$.
  Bias is observed minus predicted, so a positive value means the map
  under-predicts.}
  \label{tab:milan-holdout}
  \begin{tabular}{l S[table-format=2.3] S[table-format=2.3] S[table-format=1.3] S[table-format=-1.3]}
    \toprule
    {Predictor set} & {RMSE} & {MAE} & {$R^2$} & {Bias} \\
    \midrule
...
The ordering is the same on RMSE, on MAE and on \Rsq{}, so it is not an artefact
of a single metric. \Cref{fig:predictor-ranking} shows the three metrics side by
side, which is the form in which that claim can be checked rather than taken on
trust: the four bars fall in the same order in all three panels. The gap between
the best composite and the embeddings, 4.661 RMSE, is larger than the gap between
the best and worst composite, 1.823. All four maps are close to unbiased on this
...
\Cref{fig:holdout-scatter} shows observed against predicted values on the holdout
for the percentile composite, and \cref{fig:perclass-milan} breaks the same run
down by IMD class.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{outputs_S2_percentile_p10p25p50p75p90/fig07_holdout_scatter.png}
  \caption{Observed CLMS against predicted IMD on the 1\,014-point holdout, S2
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/fig_milan_predictor_ranking.png}
  \caption{The four Milan predictor sets on the 1\,014-point spatial holdout,
  scored against CLMS under the GEE random forest, ranked on RMSE with the best at
  the top. The same four values appear in \cref{tab:milan-holdout}. Bars start at
  zero on all three panels, so bar length is proportional to the metric and the
  spread is not exaggerated by a truncated axis. Section 7.1 returns to that
  spread, which compresses sharply under the independent validation.}
  \label{fig:predictor-ranking}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{outputs_S2_percentile_p10p25p50p75p90/figC_perclass_GEE_RF.png}
  \caption{RMSE, MAE and bias per IMD class, S2 percentile composite, GEE random
  forest, scored against CLMS.}
  \label{fig:perclass-milan}
\end{figure}

\Cref{fig:milan-raster} maps the percentile prediction against CLMS across the
whole scene. The two upper panels agree on the structure of the conurbation: the
dense core, the satellite towns to the north, and the largely agricultural south.
...
departs from CLMS in two places. The first is a fine network of positive lines
threading the countryside, following the road and canal network: the model reads
these narrow sealed features as more impervious than CLMS does. The second is a
mild negative cast over the densest part of the city centre, where the model sits
below CLMS. The predicted panel also carries small unmapped gaps along the
watercourses that CLMS fills. The map is qualitative support for
\cref{tab:milan-holdout} and not a measurement: the whole-scene comparison is not
the 1\,014-point holdout the metrics are scored on, and no number is quoted from
it.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/fig_milan_raster_comparison.png}
  \caption{Milan IMD, observed CLMS against the predicted map and their
  difference, S2 percentile composite, random forest. Positive difference means
  the model reads more impervious than CLMS.}
  \label{fig:milan-raster}
\end{figure}

\subsection{The CV-versus-holdout reversal}

SVR wins cross-validation and loses the holdout. This reproduces Žgela's result
\cite[p.~5]{zgela2026} rather than adding to it: he reports SVR best on cross-validation with
RF close behind, then RF winning every holdout metric, and selects RF for that
reason.
...
on all three of RMSE, MAE and \Rsq{}: 14.756 against 14.123 on RMSE, 11.400
against 10.675 on MAE, and 0.822 against 0.837 on \Rsq{}. The cross-validation
values are scores on training-set folds and are not comparable with the holdout
figures in \cref{tab:milan-holdout}; only the direction of each comparison is
being read across them.

The reversal carries a confound that should be stated. The holdout rows are
scored on rasters built in Earth Engine by \texttt{smileRandomForest} and
\texttt{libsvm}, while the cross-validation numbers come from the tuned
scikit-learn models, and the parameter translation between the two is lossy in
both directions. The reversal may therefore reflect overfitting to the
cross-validation folds, or it may reflect how faithfully each Earth Engine
estimator reproduces its scikit-learn counterpart. The two cannot be separated
from these runs. Consistently with this, the embeddings run records SVR as its
cross-validation winner in metadata, while RF is the model carried downstream.
...
is supported in Earth Engine through \texttt{ee.Classifier.libsvm} \cite{chang2011libsvm} with
\texttt{svmType='EPSILON\_SVR'}, and the SVR raster for Milan was produced and
retained.

\subsection{Where the signal is}
...
\Cref{tab:band-importance} gives the five most important bands of the percentile
run by permutation importance on the holdout, with mean decrease in impurity
alongside, and \cref{fig:band-importance} gives the ranking across all fifty
bands.

\begin{table}[htbp]
  \centering
  \caption{Top five bands by permutation importance, S2 percentile composite,
  random forest, Milan holdout.}
  \label{tab:band-importance}
  \begin{tabular}{c l S[table-format=1.4] S[table-format=1.4]}
    \toprule
    {Rank} & {Band} & {Permutation importance} & {Impurity importance} \\
    \midrule
    1 & B4\_p25 & 0.1812 & 0.3518 \\
    2 & B4\_p10 & 0.0556 & 0.1163 \\
    3 & B8\_p90 & 0.0401 & 0.0664 \\
    4 & B4\_p50 & 0.0225 & 0.0751 \\
    5 & B8\_p75 & 0.0211 & 0.0446 \\
    \bottomrule
  \end{tabular}
\end{table}

B4\_p25 alone carries more than three times the permutation importance of the
...
\cite[p.~6, Fig.~7]{zgela2026}, which have no physical interpretation. The Sentinel-2
composites give an interpretable answer to why they work; the embeddings do not.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{outputs_S2_percentile_p10p25p50p75p90/figD_importance_RF.png}
  \caption{Impurity and permutation importance per band, random forest, S2
  percentile composite.}
  \label{fig:band-importance}
\end{figure}

\section{Vietnam results --- same-source validation}
...
\subsection{Zero-shot versus local retrain}

\Cref{tab:vietnam-holdout} gives both scenarios for both predictor sets in both
cities.

\begin{table}[htbp]
  \centering
  \caption{Vietnam holdout metrics against GHS-BUILT-S, random forest. Bias is
  observed minus predicted, so a negative value means the map over-predicts.}
  \label{tab:vietnam-holdout}
  \begin{tabular}{l l l S[table-format=2.3] S[table-format=2.3] S[table-format=-1.3] S[table-format=-2.3] c}
    \toprule
    {City} & {Predictor set} & {Scenario} & {RMSE} & {MAE} & {$R^2$} & {Bias} & {$n$} \\
    \midrule
...
The zero-shot \Rsq{} is at or below zero in three of the four city and predictor
combinations: $-0.070$ and $-0.142$ in Hanoi, $-0.279$ for the embeddings in HCMC.
A model with \Rsq{} at or below zero is no more informative than predicting the
mean of the reference everywhere. The fourth combination, the S2 median in HCMC
at 0.042, is not a counter-example so much as a weaker instance of the same
failure.

Local retraining recovers \Rsq{} to between 0.522 and 0.649 in every case, and
cuts RMSE by roughly a third to a half: from 35.960 to 23.860 and 37.150 to
24.050 in Hanoi, and from 40.650 to 21.310 and 35.190 to 22.880 in HCMC. The
improvement holds for both predictor sets and in both cities, which is what makes
the conclusion a statement about transfer rather than about a particular feature
space.
...
\Cref{fig:transfer-emb,fig:transfer-s2median} show the two predictor sets
separately, on identical axes. They are the same experiment run on different
features, so they are read as a pair: the shape of the result is the same in
both, which is the point of Section 5.1.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{outputs_transfer_v2/fig01_transfer_comparison.png}
  \caption{\textbf{AlphaEarth embeddings.} Milan baseline, zero-shot transfer and
  ...
  map.}
  \label{fig:transfer-emb}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{outputs_transfer_S2_median/fig01_transfer_comparison.png}
  \caption{\textbf{S2 median composite.} The same four metrics and the same two
  scenarios as \cref{fig:transfer-emb}, for the Sentinel-2 median predictor set
  rather than the embeddings, and scored against the same targets: GHS-BUILT-S in
  Hanoi and HCMC, CLMS for the Milan baseline. Again agreement with the target,
  not accuracy.}
  \label{fig:transfer-s2median}
\end{figure}

\Cref{fig:transfer-raster} shows the same three maps as rasters, where the level
shift the metrics report is visible directly.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{outputs_transfer_v2/fig_obs_vs_pred_hanoi_hcmc.png}
  \caption{GHS-BUILT-S, the Milan zero-shot transfer and the local retrain as
  rasters, Hanoi and HCMC, \textbf{AlphaEarth embeddings}, scored against
  GHS-BUILT-S.}
  \label{fig:transfer-raster}
\end{figure}
...
retraining and not noise. Žgela reports the same pattern \cite[p.~9]{zgela2026}, with
...
Per-class \Rsq{} is available but is not quoted here. Restricting to a single IMD
class removes most of the variance that \Rsq{} is normalised by, so every defined
per-class value is negative even where the same model reaches a global \Rsq{} of
0.522 to 0.649. That is arithmetic rather than model failure. Classes C0 and C6
are single-valued strata, so their variance is exactly zero and their \Rsq{} is
undefined. Per-class RMSE, MAE and bias are the appropriate within-class
measures, and \Rsq{} is reserved for the global comparison in Section 5.1.

\Cref{fig:perclass-mae} shows the pattern across all seven classes.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{outputs_transfer_v2/fig02_per_class_mae.png}
  \caption{MAE per IMD class, zero-shot transfer against local retrain, Hanoi and
  HCMC, \textbf{AlphaEarth embeddings}, scored against GHS-BUILT-S. The S2 median
  figures quoted in this subsection are read from that run's per-class table
  rather than from a second copy of this figure.}
  \label{fig:perclass-mae}
\end{figure}
...
\subsection{All fifteen maps}

\Cref{tab:independent-all} gives every map under the primary strict rule.

\begin{table}[htbp]
  \centering
  \caption{All fifteen maps against 450 photo-interpreted plots per city, strict
  ...
  interpreted reference. $\mathrm{RMSE}_{\mathrm{corr}}$ is the binomial
  reference-noise correction and is an upper bound on map error rather than a
  point estimate.}
  \label{tab:independent-all}
  \begin{tabular}{l l l S[table-format=2.3] S[table-format=2.3] S[table-format=2.3] S[table-format=1.3] S[table-format=-2.3]}
    \toprule
    {City} & {Map} & {Role} & {RMSE} & {$\mathrm{RMSE}_{\mathrm{corr}}$} & {MAE} & {$R^2$} & {Bias} \\
    \midrule
    Milan & S2 stack           & model     & 24.644 & 21.559 & 16.551 & 0.652 & -3.344  \\
    Milan & S2 percentile      & model     & 24.809 & 21.874 & 16.486 & 0.647 & -3.342  \\
    Milan & emb\_RF            & model     & 25.625 & 22.420 & 18.299 & 0.624 & -1.622  \\
    Milan & S2 median          & model     & 25.984 & 22.917 & 18.363 & 0.613 & -4.176  \\
    Milan & CLMS               & reference & 26.253 & 24.411 & 14.606 & 0.605 & 2.263   \\
    Hanoi & emb\_localrf       & model     & 26.502 & 23.266 & 19.588 & 0.649 & 6.592   \\
    Hanoi & S2\_median\_localrf & model    & 27.282 & 23.885 & 21.320 & 0.628 & 8.181   \\
    Hanoi & S2\_median\_zeroshot & model   & 33.791 & 31.195 & 24.469 & 0.429 & -15.876 \\
    Hanoi & emb\_zeroshot      & model     & 34.352 & 31.061 & 29.537 & 0.410 & -15.716 \\
    Hanoi & GHSL               & reference & 40.345 & 38.833 & 27.052 & 0.187 & 19.680  \\
    HCMC  & S2\_median\_zeroshot & model   & 25.950 & 23.420 & 16.779 & 0.665 & -6.827  \\
    HCMC  & emb\_localrf       & model     & 27.242 & 24.011 & 20.294 & 0.630 & 11.088  \\
    HCMC  & S2\_median\_localrf & model    & 29.615 & 26.484 & 23.030 & 0.563 & 12.233  \\
    HCMC  & GHSL               & reference & 37.355 & 35.597 & 25.730 & 0.305 & 19.800  \\
    HCMC  & emb\_zeroshot      & model     & 38.494 & 35.775 & 32.404 & 0.262 & -19.641 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\Cref{fig:forest-ci} is the summary view, and carries the bootstrap confidence
intervals that the table omits. The Milan intervals overlap one another
substantially, and the Milan models' intervals overlap CLMS's.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{outputs_validation/fig02_forest_ci.png}
  \caption{RMSE and MAE with 95\,\% percentile bootstrap \cite{efron1993bootstrap} confidence intervals
  for all fifteen maps, three cities, strict rule. Intervals are over 10\,000
  resamples of the 450 plots, the plot being the independent unit. Diamonds mark
  the training products, scored here as maps rather than as targets; the tick on
  each RMSE bar is the noise-corrected RMSE.}
  \label{fig:forest-ci}
\end{figure}
...
any Milan map at 14.606 alongside the worst RMSE at 26.253.
\Cref{tab:milan-error-dist} gives the per-plot error distribution behind that
split. CLMS is right far more often than any model, landing within 5 percentage
points of the interpretation on 53.3\,\% of plots against 34.2\,\% for the closest
model, and wrong by more when it is wrong, carrying the largest share of gross
errors at 8.7\,\% of plots above 50 percentage points. That is a product resolving
each plot decisively where the models fitted to it hedge, which costs them where
CLMS is nearly exact and saves them where it fails badly; MAE rewards the first
behaviour and RMSE penalises the second, so the two metrics measure the two
halves of one trade rather than contradicting each other.

\begin{table}[htbp]
  \centering
  \caption{Distribution of per-plot absolute error, Milan, strict rule,
  $n = 450$. ``Within 5 pp'' and ``over 50 pp'' are the percentages of plots whose
  absolute error falls below and above those thresholds.}
  \label{tab:milan-error-dist}
  \setlength{\tabcolsep}{4pt}
  \begin{tabular}{l S[table-format=2.2] S[table-format=2.2] S[table-format=2.2] S[table-format=2.2] S[table-format=3.2] S[table-format=2.1] S[table-format=1.1]}
    \toprule
    {Map} & {MAE} & {RMSE} & {\shortstack{Median\\error}} & {p90} & {Max} & {\shortstack{Within\\5 pp (\%)}} & {\shortstack{Over\\50 pp (\%)}} \\
    \midrule
    S2 stack      & 16.55 & 24.64 & 10.38 & 39.37 & 96.26  & 32.7 & 6.7 \\
    S2 percentile & 16.49 & 24.81 & 9.09  & 40.76 & 94.92  & 34.2 & 6.7 \\
    emb\_RF       & 18.30 & 25.63 & 14.38 & 43.67 & 82.84  & 29.8 & 7.3 \\
...
\Cref{fig:scatter-grid} gives the plot-level view behind these summaries, every
registered map in all three cities on common axes. It is the one place the
scatter itself is shown rather than reduced to a metric, and the shape of the
Vietnamese zero-shot panels, mass held away from the 1:1 line at low reference
values, is the distributional failure Section 7.2 measures.

\begin{landscape}
\begin{figure}[p]
  \centering
  % pdflscape rotates the page but does NOT swap \textwidth and \textheight:
  % inside this environment \textwidth (455pt) is still the measure the image
  % is laid out against, and \textheight (703pt) is the long side the rotated
  % page actually offers. Asking for width=\textheight therefore silently
  % clamps to \textwidth. Scaling by \textwidth keeps the 45-panel grid on one
  % ordinary A4 page -- the page stays A4 and only its content is rotated.
  \includegraphics[width=\textwidth,keepaspectratio]%
    {outputs_validation/fig01_scatter_grid.png}
  \caption{Photo-interpreted reference against predicted IMD, every registered
  map in the three cities, strict rule.}
  \label{fig:scatter-grid}
\end{figure}
\end{landscape}

\subsection{Two tiers under paired testing}

The Milan four are not a clean ranking of four maps but two tiers of two: S2
stack with S2 percentile, and emb\_RF with S2 median, indistinguishable within
...
\Cref{tab:paired-wilcoxon} gives the six Milan pairs, tested by paired Wilcoxon
signed-rank \cite{wilcoxon1945} on per-plot absolute error with Benjamini-Hochberg control of
the false discovery rate \cite{benjamini1995fdr} within the city.

\begin{table}[htbp]
  \centering
  \caption{Paired Wilcoxon tests on per-plot absolute error, Milan, strict rule,
  $n = 450$ plots. Median difference is the median of the paired per-plot
  differences in absolute error.}
  \label{tab:paired-wilcoxon}
  \setlength{\tabcolsep}{4pt}
  \begin{tabular}{l l S[table-format=2.2] S[table-format=2.2] S[table-format=-1.2] l l c}
    \toprule
    {Map A} & {Map B} & {MAE A} & {MAE B} & {\shortstack{Median\\difference}} & {$p$} & {$q$ (BH)} & {\shortstack{Distin-\\guishable}} \\
    \midrule
    S2 median & S2 percentile & 18.36 & 16.49 & 0.92  & 5.95e-09 & 3.57e-08 & yes \\
    S2 median & S2 stack      & 18.36 & 16.55 & 1.07  & 4.33e-05 & 1.30e-04 & yes \\
    emb\_RF   & S2 stack      & 18.30 & 16.55 & 0.60  & 9.62e-04 & 1.92e-03 & yes \\
    emb\_RF   & S2 percentile & 18.30 & 16.49 & 0.31  & 4.56e-03 & 6.84e-03 & yes \\
    S2 stack  & S2 percentile & 16.55 & 16.49 & 0.19  & 6.55e-01 & 7.86e-01 & no  \\
    emb\_RF   & S2 median     & 18.30 & 18.36 & -0.64 & 7.90e-01 & 7.90e-01 & no  \\
    \bottomrule
  \end{tabular}
\end{table}

Each of S2 stack and S2 percentile separates from each of emb\_RF and S2 median,
with $q$ between 3.57e-08 and 6.84e-03. Neither of the two within-tier pairs
separates: $q = 0.79$ for S2 stack against S2 percentile, and $q = 0.79$ for
emb\_RF against S2 median.
...
\subsection{Rule sensitivity}

The conclusion does not depend on how the impervious classes are coded.
\Cref{tab:rule-sensitivity} gives the Milan RMSE under all three rules with the
resulting ranks.

\begin{table}[htbp]
  \centering
  \caption{Milan RMSE against photo-interpretation under the three impervious
  ...
  \begin{tabular}{l S[table-format=2.2] S[table-format=2.2] S[table-format=2.2] c c c}
    \toprule
    {Map} & {Strict} & {B} & {C} & {Rank strict} & {Rank B} & {Rank C} \\
    \midrule
    S2 stack      & 24.64 & 24.54 & 20.98 & 1 & 1 & 1 \\
    S2 percentile & 24.81 & 24.71 & 21.37 & 2 & 2 & 2 \\
    emb\_RF       & 25.63 & 25.54 & 23.03 & 3 & 3 & 4 \\
...
Under rule C the top two maps and CLMS hold their positions, while emb\_RF and S2
...
\subsection{From same-source to independent validation}
...
that is a compression of about 6.4 times. \Cref{fig:samesource-vs-independent}
shows the four maps moving between the two validations.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{figs/fig_samesource_vs_independent.png}
  \caption{The four Milan predictor sets, same-source RMSE against CLMS on the
  ...
  \label{fig:samesource-vs-independent}
\end{figure}
...
\subsection{Why transfer fails: level matching and the lost low tail}
...
\begin{table}[htbp]
  \centering
  \caption{Low-tail and spread retention for the eight Vietnamese maps, computed
  ...
  of magnitude rather than to the percentage point, and a value at or above
  100\,\% means the tail is fully present rather than oversized.}
  \label{tab:tail-spread}
  \begin{tabular}{l l l S[table-format=3.1] S[table-format=2.0]}
    \toprule
    {City} & {Map} & {Scenario} & {Tail retained (\%)} & {Spread retained (\%)} \\
    \midrule
    Hanoi & emb\_zeroshot        & zero-shot     & 3.1   & 37 \\
    Hanoi & S2\_median\_zeroshot & zero-shot     & 30.6  & 53 \\
    Hanoi & emb\_localrf         & local retrain & 136.0 & 61 \\
    Hanoi & S2\_median\_localrf  & local retrain & 124.4 & 51 \\
    HCMC  & emb\_zeroshot        & zero-shot     & 0.0   & 26 \\
    HCMC  & S2\_median\_zeroshot & zero-shot     & 68.7  & 75 \\
    HCMC  & emb\_localrf         & local retrain & 108.3 & 57 \\
    HCMC  & S2\_median\_localrf  & local retrain & 104.0 & 51 \\
    \bottomrule
  \end{tabular}
\end{table}
...
distributional counterpart of the \Rsq{} recovery reported in Section 5.1.
...
\Cref{fig:hcmc-histogram} carries the argument for HCMC: the predicted
distributions for the four maps against the reference, with the level means
annotated.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/fig_hcmc_prediction_histogram.png}
  \caption{Distribution of predicted IMD over the 450 HCMC plots for the four
  ...
  severe case, but \cref{tab:tail-spread} records that the tail is lost in Hanoi
  as well.}
  \label{fig:hcmc-histogram}
\end{figure}

One comparison this section does not make is between the two Hanoi S2 median
scenarios. The paired test of S2\_median\_zeroshot against S2\_median\_localrf in
...
\subsection{Retraining works: bias recovery}
...
The effect itself is not new here. Žgela reports it qualitatively \cite[p.~10]{zgela2026},
...
\Cref{tab:bias-recovery} gives the measurement.

\begin{table}[htbp]
  \centering
  \caption{Bias against photo-interpretation for GHS-BUILT-S and for the local
  ...
  \label{tab:bias-recovery}
  \begin{tabular}{l l S[table-format=2.2] l S[table-format=2.2] S[table-format=2.2] S[table-format=2.1]}
    \toprule
    {City} & {Target} & {Target bias} & {Map} & {Map bias} & {Recovered (pp)} & {Recovered (\%)} \\
    \midrule
    Hanoi & GHSL & 19.68 & emb\_localrf        & 6.59  & 13.09 & 66.5 \\
    Hanoi & GHSL & 19.68 & S2\_median\_localrf & 8.18  & 11.50 & 58.4 \\
    HCMC  & GHSL & 19.80 & emb\_localrf        & 11.09 & 8.71  & 44.0 \\
    HCMC  & GHSL & 19.80 & S2\_median\_localrf & 12.23 & 7.57  & 38.2 \\
    \bottomrule
  \end{tabular}
\end{table}
...
the S2 median in both cities, but it is substantial in all four cases.
\Cref{fig:bias-recovery} shows the four movements.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{figs/fig_bias_recovery.png}
  \caption{Bias against photo-interpretation, from GHS-BUILT-S to each local
...
target's systematic error the models do inherit. The claim is bounded accordingly.
A model can score better than its own training target, and here it measurably
does, but it is not thereby freed from that target: retraining on a product that
...
\textbf{Composite depth is confounded with predictor type in Vietnam.} The Milan
...
\textbf{Plot-level independence is assumed and untested.} The 450 plots in each city
...
below 450, the bootstrap intervals in \cref{fig:forest-ci} are narrower than they
should be, and the paired $q$ values of Section 6.2 are optimistic. This bears
most directly on the two-tier result. The tier separation runs from
$q = 3.57$e-08 to $q = 6.84$e-03, and the stronger of those would survive a
considerable inflation of the standard errors, but the weakest would not
necessarily survive a large one. The safe reading is that the tier structure is
supported and that its weakest pair should not be treated as established at the
quoted precision.

\textbf{$\mathrm{RMSE}_{\mathrm{corr}}$ is an upper bound on map error, not a point
estimate.} The correction subtracts a binomial estimate of reference noise built
on the assumption that the nine sub-cells of a plot are independent draws. They
are not: sub-cells within a single 10\,m pixel share land cover and are spatially
correlated, so the binomial variance overstates nothing but understates the true
reference noise, and the correction consequently removes less error than it
should. $\mathrm{RMSE}_{\mathrm{corr}}$ therefore bounds map error from above and
cannot be quoted as the error of any particular map. Section 6.1's statement that
the four Milan maps lie between 21.559 and 22.917 corrected is a statement about
bounds, and the claim that any of them achieves an error of 21.559 is not
supported.

\textbf{A single interpreter produced the reference, and there is no inter-rater
...
applies anywhere else. The absolute RMSE and MAE levels of
\cref{tab:independent-all} carry an unquantified interpreter component in full,
as do the bootstrap intervals of \cref{fig:forest-ci} and the bias measurements
of Section 7.3, because each of those scores a map against the reference rather
than against another map. Nor does the shared reference protect the rankings: a
tendency that displaces the reference by different amounts in different kinds of
plot penalises maps unequally, so the ordering in \cref{tab:independent-all} is
not immune either. The single claim the shared reference supports is that the
paired within-city differences are less exposed to interpreter bias than the
error levels are.

\textbf{The plots are stratified by the product under test, so the metrics are not
...
\textbf{1. Composite choice matters more than the foundation model, on the same-source
...
sets is the same on RMSE, MAE and \Rsq{}. The advantage is mechanistically
interpretable rather than merely empirical: \texttt{B4\_p25} alone carries a permutation
...
\textbf{2. Against an independent reference the four-way spread compresses about
...
nonetheless real: S2 stack and S2 percentile each separate from emb\_RF and S2
...
\textbf{3. Zero-shot transfer fails and local retraining is necessary.} The zero-shot
\Rsq{} is at or below zero in three of the four city and predictor combinations,
...
\textbf{4. The training products are not ceilings.} CLMS scores an RMSE of 26.253
...
% References. The entries live in refs.bib; unsrt numbers them in order of
% first citation, which reproduces the [1]..[15] order the prose was written
% against. \renewcommand states the heading explicitly so that a change of
% documentclass cannot silently retitle it "Bibliography".
